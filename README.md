...
All'interno del file **[RISULTATI.md](RISULTATI.md)** si trovano **250 esecuzioni complete, 25 per ogni configurazione**, con il confronto tra strategie di aggregazione, quattro modi diversi di dividere gli indirizzi fra i client, e l'analisi di dove il modello federato peggiora rispetto a quello centralizzato.

**In breve, la configurazione migliore è FedAvg con fraction-train = 0.5 e lr-decay = 0.97**, con f1-score 0.6101 contro lo 0.7615 del centralizzato. Il divario è dovuto per due terzi alla classe **server** e per quasi un terzo ai **net-device**. La differenza fra `fraction-train` 0.3 e 0.5 non esiste, il decadimento del learning rate aiuta invece di peggiorare, e FedProx è la seconda strategia migliore.
...
Aggiungere qualche net-device ai client che ne hanno pochi non basta (provato condividendo lo 0.5% del pool, cioè 425 serie), perchè lascia intatte le proporzioni fra le classi dentro ogni client.